...
public class CreateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (Double.parseDouble(price) > 500000 || Double.parseDouble(price) < 1000) {
                productError.setPriceError("Price must be in [1000, 500000]");
                checkValidation = false;
            }
            if (Double.parseDouble(quantity) > 100 || Double.parseDouble(quantity) < 0) {
            ...
            if (checkValidation) {
                ProductDTO product = new ProductDTO(productID, productName, image, categoryID, Double.parseDouble(price), Double.parseDouble(quantity), importDate, usingDate);
